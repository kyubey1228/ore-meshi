...
// [slug, name, featured(注目), trending(急上昇)]
const articleCategorySeeds=[['dinner-partners','食事相手',true,false],['meal-friends','ご飯友達',true,false],['drinking-friends','飲み友達',false,true],['solo-dining','一人ご飯',false,true],['adult-friendship','社会人の友達作り',false,false],['gourmet','グルメ',false,false],['service-guide','サービス活用法',false,false],['safety','安全な利用方法',true,false]] as const;
...
async function main() {
  ...
  await seedSequential(articleCategorySeeds,([slug,name,featured,trending])=>prisma.articleCategory.upsert({where:{slug},create:{slug,name,featured,trending},update:{name,featured,trending}}));
