...
test('ビジネスユーザー: Dashboard・需要・分析・掲載作成導線', async ({ page }) => {
  ...
  await page.goto('/business/demand'); await expect(page.getByRole('heading')).toContainText('Demand');
  await page.goto('/business/analytics'); await expect(page.getByRole('heading')).toContainText(/分析|Analytics/);
  await page.goto('/business/seats/new'); await expect(page.getByRole('heading')).toContainText(/席|空席/);
});
