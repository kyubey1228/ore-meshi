...
test.describe.serial('ビジネスと複数個人ユーザー', () => {
  ...
  for (const key of ['user1','user2'] as const) test(`${key}が同じ募集へ参加する`, async ({ browser }) => { const c=await browser.newContext({storageState:statePath(key)});const p=await c.newPage();await p.goto(mealUrl);await p.getByRole('button',{name:'この募集に参加する'}).click();await expect(p.getByText(/参加申請|返事待ち/)).toBeVisible();await c.close(); });
  test('店舗オーナーが2名を順に承認し満員成立する', async ({ browser }) => { const c=await browser.newContext({storageState:statePath('business')});const p=await c.newPage();await p.goto(mealUrl);const buttons=p.getByRole('button',{name:'一緒に行く'});await buttons.first().click();await p.reload();await p.getByRole('button',{name:'一緒に行く'}).first().click();await expect(p.getByRole('heading',{name:'飯、決まった。'})).toBeVisible();await c.close(); });
