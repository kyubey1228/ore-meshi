...
test.describe.serial('ユーザー1とユーザー2', () => {
  ...
  test('ユーザー1が募集を作成し共有導線を確認する', async ({ browser }) => {
    ...
    await expect(page.getByText('Xで共有')).toBeVisible(); await expect(page.getByText('LINEで送る')).toBeVisible(); await expect(page.getByText('URLをコピー')).toBeVisible(); await expect(page.getByText('QRコード')).toBeVisible();
    await context.close();
  });
  test('ユーザー2が参加申請する', async ({ browser }) => {
    ...
    await expect(page.getByText(/参加申請|返事待ち/)).toBeVisible(); await context.close();
  });
  test('ユーザー1が承認して成立させる', async ({ browser }) => {
    ...
    await expect(page.getByRole('heading', { name: '飯、決まった。' })).toBeVisible(); await context.close();
  });
});
