...
export const mealSchema = z.object({ title: z.string().trim().min(1).max(80), area: z.string().trim().min(1).max(80), budgetMin: z.coerce.number().int().min(0).max(100000), budgetMax: z.coerce.number().int().min(0).max(100000), maxParticipants: z.coerce.number().int().min(2).max(20), paymentType: z.enum(['SPLIT','HOST_PAYS','GUEST_PAYS']), restaurant: optionalText(120), description: optionalText(1000), genre: optionalText(60), alcohol: optionalText(60), smoking: optionalText(60), ageCondition: optionalText(60), deadline: z.union([z.literal(''),z.iso.datetime({offset:true})]).optional().default(''), candidates: z.array(candidateSchema).min(1).max(10), purposeIds: z.array(idSchema).max(3).refine(ids=>new Set(ids).size===ids.length).optional().default([]) }).superRefine((v,ctx) => {
...
export const profileSchema = z.object({ displayName: z.string().trim().min(1).max(50), bio: optionalText(500), email: z.union([z.literal(''),z.string().trim().email().max(200)]).optional(), diningTypeIds: z.array(idSchema).max(5).refine(ids=>new Set(ids).size===ids.length).optional().default([]) });
export const filterSchema = z.object({ date: z.union([daySchema,z.literal('')]).optional(), area: z.string().trim().max(80).optional(), budget: z.union([z.literal(''),z.coerce.number().int().min(0).max(100000)]).optional(), paymentType: z.enum(['SPLIT','HOST_PAYS','GUEST_PAYS','']).optional(), purpose: z.string().trim().max(80).regex(/^[a-z0-9-]*$/).optional(), when: z.enum(['today','tonight','soon','']).optional(), remaining: z.union([z.literal(''),z.coerce.number().int().min(1).max(2)]).optional() });
