...
export const filterSchema = z.object({ date: z.union([daySchema,z.literal('')]).optional(), area: z.string().trim().max(80).optional(), budget: z.union([z.literal(''),z.coerce.number().int().min(0).max(100000)]).optional(), paymentType: z.enum(['SPLIT','HOST_PAYS','GUEST_PAYS','']).optional(), purpose: z.string().trim().max(80).regex(/^[a-z0-9-]*$/).optional() });
