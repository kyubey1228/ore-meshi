...
export const profileSchema = z.object({ displayName: z.string().trim().min(1).max(50), bio: optionalText(500), diningTypeIds: z.array(idSchema).max(5).refine(ids=>new Set(ids).size===ids.length).optional().default([]) });
