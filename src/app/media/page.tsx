...
export default async function MediaHome({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  return <section className="section"><GrowthTracker eventType="MEDIA_VIEW" source="media_home" />
    ...
    <div className="panel"><h2>カテゴリ</h2><div className="tag-pills">{categories.map(category => <Link className="tag-pill" key={category.id} href={`/media/category/${category.slug}`}>{category.name}</Link>)}</div></div>
