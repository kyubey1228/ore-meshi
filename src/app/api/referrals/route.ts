...
export async function POST(request:Request){
  ...
  const data=parsed.data;const campaign=await getCampaignShareData(data.kind,data.id);if(!campaign||!campaignIsShareable(campaign))return NextResponse.json({ok:false},{status:404});
