...
export default async function BusinessDashboard(){
  ...
  return <section className="section">
    <div className="section-heading"><div><span className="eyebrow orange">BUSINESS TABLE</span><h1>{data.membership.businessAccount.name}</h1><p className="muted">Xで飯を呼ぶ。</p></div><Link className="btn" href="/business/social">Xの投稿と成果を見る</Link></div>
    <BusinessCampaignForm businessAccountId={data.membership.businessAccountId}/>
    <div className="dashboard-grid">{campaigns.map(item=><article className="panel" key={`${item.kind}:${item.id}`}><span className="tag">PR · {item.status}</span><h2>{item.title}</h2><Link className="btn" href={`/business/social?kind=${item.kind}&id=${item.id}`}>Xで宣伝する</Link></article>)}{!campaigns.length&&<div className="empty"><span className="empty-icon">🏪</span><p>共有できるキャンペーンはまだありません。</p></div>}</div>
  </section>;
}
