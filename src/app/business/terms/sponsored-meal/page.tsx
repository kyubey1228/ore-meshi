...
export const metadata: Metadata = { title: 'スポンサー飯 掲載規約', description: 'スポンサー飯（募集連動型の個別出稿商品）の掲載に関する規約です。' };

const articles = [
  ['第1条（商品概要）', 'スポンサー飯は、店舗・企業が飲食代や特典の一部・全部を提供し、実際の飯の募集（Meal）として「俺は誰かと飯が食いたい！」に掲載する個別出稿商品です。'],
  ['第2条（申し込みと掲載開始）', 'お申し込み内容の作成後、Stripe Checkoutでの決済が完了した時点で掲載が開始されます。決済前の下書き状態であれば、いつでも取りやめることができます。決済完了後、内容に対応する飯の募集ページが自動的に作成され、一般ユーザーに公開されます。'],
  ['第3条（掲載期間）', '掲載は、お申し込み時に設定した開催日時をもって終了します。開催日時を過ぎた掲載は、成果の確認は引き続きできますが、新たな参加希望は受け付けなくなります。'],
...
export default function SponsoredMealTermsPage() {
  return (
