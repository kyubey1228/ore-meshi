...
async function SocialProof() {
  ...
  return socialProof ? <p className="muted hero-social-proof">{socialProof.text}</p> : null;
}

async function HomeMeals() {
  const meals = await measurePerformance('HOME', 'meals', () => getMealList({}, {}, 12));
  return meals.length ? <div className="meal-grid">{meals.slice(0, 6).map(meal => <MealCard key={meal.id} meal={meal} />)}</div> : <Empty>今は誰も飯を募集してないみたい。</Empty>;
}

async function PopularAreas() {
  const popularAreas = await getPopularAreas(5);
  return popularAreas.length > 0 && <section className="section"><div className="section-heading"><div><span className="eyebrow orange">POPULAR AREAS</span><h2>今、人が集まりやすいエリア</h2><p className="muted">直近30日で実際に飯が成立した実績にもとづく表示です。</p></div></div><div className="area-bars">{popularAreas.map(a => <Link key={a.area} href={`/recruitments/${encodeURIComponent(a.area)}`} className="area-bar-row"><span>{a.area}</span><span className="muted">直近30日{a.completedMeals30d}件成立・成立率{Math.round(a.fillRate * 100)}%</span><ArrowRight size={16} /></Link>)}</div></section>;
}
