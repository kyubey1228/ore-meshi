...
export default async function Home() {
  const [meals, userId, popularAreas] = await Promise.all([getMealList(), currentUserId(), getPopularAreas(5)]);
  return <>
    <section className="hero"><div className="hero-copy"><span className="eyebrow"><span className="live-dot" />ひとり飯もいいけど、今日は誰かと。</span><h1>俺は誰かと<br /><span className="underline-orange">飯が食いたい</span><span className="orange">！</span></h1><p className="hero-catch">今日、誰かと飯食わない？</p><p className="muted hero-description">行きたいお店がある。ちょっと誰かと話したい。<br />理由はそれくらいでいい。気軽に、飯の仲間を見つけよう。</p><div className="hero-actions"><Link className="btn" href="/meals/new">飯を募集する <ArrowRight size={18} /></Link><Link className="btn secondary" href="/meals">誰かの飯に乗っかる</Link></div>{!userId && <Link className="login-hint" href="/login">𝕏 Twitter/Xで気軽にはじめる →</Link>}</div><div className="hero-art" aria-label="お箸とごはん、誰か飯いかん？"><span className="art-note">誰か、飯いかん？</span><div className="art-circle"><span className="rice">🍚</span><span className="chopsticks">🥢</span></div><span className="art-caption">GOOD FOOD, GOOD COMPANY.</span><span className="spark spark-one">✳</span><span className="spark spark-two">✳</span></div></section>
    ...
    {popularAreas.length > 0 && <section className="section"><div className="section-heading"><div><span className="eyebrow orange">POPULAR AREAS</span><h2>今、人が集まりやすいエリア</h2><p className="muted">直近30日で実際に飯が成立した実績にもとづく表示です。</p></div></div><div className="area-bars">{popularAreas.map(a => <Link key={a.area} href={`/meals?area=${encodeURIComponent(a.area)}`} className="area-bar-row"><span>{a.area}</span><span className="muted">直近30日{a.completedMeals30d}件成立・成立率{Math.round(a.fillRate * 100)}%</span><ArrowRight size={16} /></Link>)}</div></section>}
