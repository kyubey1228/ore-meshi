...
// 一覧の行ごとにIntlのロケール情報を構築し直さず、サーバー・ブラウザの両方で再利用する。
const dateFormatter = new Intl.DateTimeFormat('ja-JP', { timeZone: 'Asia/Tokyo', month: 'numeric', day: 'numeric', weekday: 'short' });
const dateTimeFormatter = new Intl.DateTimeFormat('ja-JP', { timeZone: 'Asia/Tokyo', year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' });
export function dateLabel(date: Date | string) { return dateFormatter.format(new Date(date)); }
export function dateTimeLabel(date: Date | string) { return dateTimeFormatter.format(new Date(date)); }
