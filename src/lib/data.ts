...
export const publicUser = { id:true, twitterUsername:true, displayName:true, image:true, bio:true, createdAt:true } satisfies Prisma.UserSelect;
...
export async function getMealList(input: unknown = {}) {
  ...
  return prisma.meal.findMany({where,orderBy:{createdAt:'desc'},take:100,include:{host:{select:publicUser},candidates:{orderBy:{date:'asc'}},_count:{select:{joinRequests:{where:{status:'ACCEPTED'}}}}}});
}
export async function getMealById(raw: string){
  ...
  const meal=await prisma.meal.findUnique({where:{id:id.data},include:{host:{select:publicUser},candidates:{orderBy:{date:'asc'}},_count:{select:{joinRequests:{where:{status:'ACCEPTED'}}}}}});
  if(!meal)return null;
