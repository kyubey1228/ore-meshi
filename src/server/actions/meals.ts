...
export async function createMeal(input: unknown) { return perform(async userId => {
  const {candidates,deadline,...meal}=mealSchema.parse(input);
  return transaction(async tx=>{
    const result=await tx.meal.create({data:{...meal,deadline:deadline?new Date(deadline):null,hostId:userId,candidates:{create:candidates.map(c=>({...c,date:new Date(c.date)}))}}});
...
export async function updateMeal(input: unknown) { return perform(async userId=>{
  ...
  const {candidates,deadline,...data}=mealSchema.parse(raw);
  return transaction(async tx=>{
    ...
    await tx.meal.update({where:{id},data:{...data,deadline:deadline?new Date(deadline):null,candidates:{create:candidates.map(c=>({...c,date:new Date(c.date)}))}}});
    return `/meals/${id}`;
  });
}); }
