...
type CreatedMeal={href:string;title:string;area:string};
...
export function MealForm({initial,id}:{initial?:Fields & {candidates:Candidate[]};id?:string}){
  ...
  function finishCreation(shareOnX:boolean){
    ...
    if(shareOnX){
      ...
      const params=new URLSearchParams({
        text:`「${createdMeal.title}」の飯仲間を募集中！\n場所：${createdMeal.area}\n\n${url}`,
        hashtags:'俺は誰かと飯が食いたい',
      });
  ...
  return <>
    <form className="panel meal-form" onSubmit={handleSubmit(values=>{
      const parsed=mealSchema.safeParse({...values,deadline:values.deadline?new Date(`${values.deadline}:00+09:00`).toISOString():'',candidates});
      ...
      transition(async()=>{
        try{
          ...
          setCreatedMeal({href:result.href,title:parsed.data.title,area:parsed.data.area});
